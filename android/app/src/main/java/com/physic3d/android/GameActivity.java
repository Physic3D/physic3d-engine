...
public class GameActivity extends Activity {
	...
	public static AssetManager getAssets( boolean isEngine )
	{
	...
	public static boolean performEngineKeyEvent( int action, int keyCode, KeyEvent event )
	{
		...
		/*else if( action == KeyEvent.ACTION_MULTIPLE )
		{
			...
			{
				GameActivity.nativeString( event.getCharacters() );
			}
			else
			{
				// maybe unneeded
	...
	{
		mSingleton.startActivity( new Intent( Intent.ACTION_VIEW, Uri.parse("https://github.com/tyabus/xash3d/releases/latest" ) ) ); // TODO: update URL to Physic3D releases
	}
	...
	public boolean InitGL( int stencilBits ) 
	{
		try
		{
			...
			if( !egl.eglChooseConfig( dpy, configSpec[GameActivity.mPixelFormat], configs, 1, num_config ) || num_config[0] == 0 )
			{
				...
				configSpec[GameActivity.mPixelFormat][1] = 0; // disable stencil
				if( !egl.eglChooseConfig( dpy, configSpec[GameActivity.mPixelFormat], configs, 1, num_config ) || num_config[0] == 0 )
				{
					Log.e( TAG, "No EGL config available" );
					return false;
...
class GameInputConnection extends BaseInputConnection 
{
