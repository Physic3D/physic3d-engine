...
template<typename T>
static inline T* LumpedPointer( byte* base, const dlump_t* l ) {
	return reinterpret_cast<T*>( base + l->fileofs );
}

// Use LumpedPointer<T>(mod_base, l) directly at each call site
...
static void Mod_LoadVisibility( const dlump_t *l )
{
	...
	Q_memcpy( loadmodel->visdata, (byte *)(mod_base + l->fileofs), l->filelen );
