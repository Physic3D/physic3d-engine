...
void Sequence_ResetDefaults( sequenceCommandLine_s *destination, sequenceCommandLine_s *source )
{
	if( !source )
	{
		...
		{
			0, // effect
			(byte)-1, (byte)-1, (byte)-1, (byte)-1, // rgba1
			(byte)-1, (byte)-1, (byte)-1, (byte)-1, // rgba2
			0.5, 0.5, // xy
			0.2, 0.2, // fade-in/out
			1.6, // holdtime
			1.0, // fxtime
...
{
	sequenceCommandLine_s *scan, *copy, *newCmd, *prev;
	...
	for( scan = list; scan; scan = scan->nextCommandLine )
	{
		...
		{
			newCmd = Sequence_CopyCommand( scan );

			if( prev )
			{
				prev->nextCommandLine = newCmd;
				prev                  = newCmd;
			}
			else
			{
