...
void NET_SendPacket( netsrc_t sock, size_t length, const void *data, netadr_t to )
{
	...
	{
		char buf[256];
		Q_strncpy( buf, (const char *)data,  min( 256, length ));
	...
	ret = pSendTo( net_socket, (const char *)data, (int)length, 0, &addr, sizeof( addr ));
...
#ifdef _WIN32
	if( pBind( net_socket, (struct sockaddr *)&addr, sizeof( addr )) == SOCKET_ERROR )
#else
	if( pBind( net_socket, (struct sockaddr *)&addr, sizeof( addr )) < 0 )
#endif
	{
