...
void HPAK_AddToQueue( const char *name, resource_t *DirEnt, void *data, file_t *f )
{
	hpak_t	*ptr;
...
void HPAK_CreatePak( const char *filename, resource_t *DirEnt, void *data, file_t *f )
{
	int		filelocation;
	string		pakname;
	char		md5[16];
	...
	{
		MD5Update( &MD5_Hash, (const byte *)data, DirEnt->nDownloadSize );
	}
...
void HPAK_AddLump( qboolean add_to_queue, const char *name, resource_t *DirEnt, void *data, file_t *f )
{
