...
int mpg123_decode( mpg123_handle_t *mh, const byte *inmemory, size_t inmemsize, byte *outmemory, size_t outmemsize, size_t *done )
{
	...
	while( ret == MPG123_OK )
	{
		...
		if( mh->buffer.fill )
		{
			int	a = (int)(mh->buffer.fill > (outmemsize - mdone) ? outmemsize - mdone : mh->buffer.fill);
...
static const char *mpg123_error[] =
{
	...
	"Out of memory -- some (void*)malloc() failed. (code 7)",
...
extern "C" const char *get_error( void *mpeg )
{
	mpg123_handle_t *mh = (mpg123_handle_t *)mpeg;
