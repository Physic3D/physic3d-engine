...
public class ShortcutActivity extends Activity
{
	...
	public void saveShortcut(View view)
	{
		...
		intent.setAction("com.physic3d.android.START");
		if(argv.length() != 0) intent.putExtra("argv",argv.getText().toString());
		if(pkgname.length() != 0)
		{
			intent.putExtra("gamelibdir", "/data/data/"+pkgname.getText().toString().replace("!","com.physic3d.android.")+"/lib/");
			intent.putExtra("pakfile", "/data/data/"+pkgname.getText().toString().replace("!","com.physic3d.android.")+"/files/extras.pak");
		}
		...
		catch(Exception e)
		{
			...
			icon = BitmapFactory.decodeResource(getResources(), R.drawable.physic3d);
		}
